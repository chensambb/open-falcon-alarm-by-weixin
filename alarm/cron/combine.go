...
func combineWeixin() {
	dtos := popAllWeixinDto()
	count := len(dtos)
	if count == 0 {
		return
	}

	dtoMap := make(map[string][]*WeixinDto)
	for i := 0; i < count; i++ {
		key := fmt.Sprintf("%d%s%s%s", dtos[i].Priority, dtos[i].Status, dtos[i].Im, dtos[i].Metric)
		if _, ok := dtoMap[key]; ok {
			dtoMap[key] = append(dtoMap[key], dtos[i])
		} else {
			dtoMap[key] = []*WeixinDto{dtos[i]}
		}
	}
	...
	for _, arr := range dtoMap {
		size := len(arr)
		if size == 1 {
			redi.WriteWeixin([]string{arr[0].Im}, arr[0].Content)
			continue
		}

		contentArr := make([]string, size)
		for i := 0; i < size; i++ {
			log.Println("combineWeixin i:", i, " ,Content:", arr[i].Content)
			contentArr[i] = arr[i].Content
		}
		content := strings.Join(contentArr, "\r\n")
		log.Println("combineWeixin content:", content)

		redi.WriteWeixin([]string{arr[0].Im}, content)
	}
...
func popAllWeixinDto() []*WeixinDto {
	ret := []*WeixinDto{}
	...
	for {
		...
		if err != nil {
			if err != redis.ErrNil {
				log.Println("get WeixinDto fail", err)
			}
			break
		}
		...
		var weixinDto WeixinDto
		err = json.Unmarshal([]byte(reply), &weixinDto)
		if err != nil {
			log.Printf("json unmarshal WeixinDto: %s fail: %v", reply, err)
			continue
		}

		ret = append(ret, &weixinDto)
	}
